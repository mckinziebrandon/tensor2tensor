...
def build_input_fn(mode,
    ...
    def input_fn():
                ...
                with tf.name_scope("problem_%d" % n):
                    with tf.device("/cpu:0"):  # Input reading on CPU
                        ...
                        feature_map = data_reader.input_pipeline(
                            problem_instance,
                            data_file_patterns and data_file_patterns[n],
        ...
        if mode == tf.contrib.learn.ModeKeys.INFER:
            rand_inputs._shape = tf.TensorShape(
                [None, None, None, None])  # pylint: disable=protected-access
            rand_target._shape = tf.TensorShape(
                [None, None, None, None])  # pylint: disable=protected-access
