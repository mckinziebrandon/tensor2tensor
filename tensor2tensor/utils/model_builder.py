...
def build_model_fn(model, hparams):
    ...
    def initializer():
        if hparams.initializer == "orthogonal":
            ...
            return init_ops.variance_scaling_initializer(
                hparams.initializer_gain, mode="fan_avg",
                distribution="uniform")
        else:
            raise ValueError(
                "Unrecognized initializer: %s" % hparams.initializer)
    ...
    def learning_rate_decay():
        ...
        if hparams.learning_rate_decay_scheme == "noam":
            ...
            return 0.5 * (
            1 + tf.cos(np.pi * (step % cycle_steps) / cycle_steps))
    ...
    def model_fn(features, targets, mode):
        ...
        if mode == tf.contrib.learn.ModeKeys.INFER:
            if FLAGS.decode_interactive:
                features = _interactive_input_tensor_to_features_dict(features,
                                                                      my_hp)
            elif FLAGS.decode_from_file:
                features = _decode_input_tensor_to_features_dict(features,
                                                                 my_hp)
        ...
        def nth_model(n):
            ...
            return [
                       total_loss] + sharded_logits  # Need to flatten for cond later.
        ...
        with tf.name_scope("training_stats"):
            ...
            for n in xrange(len(my_hp.problems)):
                ...
                with tf.variable_scope("losses_avg", reuse=True):
                    total_loss_var = tf.get_variable(
                        "problem_%d/total_loss" % n)
        ...
        for v_name in sorted(list(all_weights)):
            ...
            if my_hp.weight_decay > 0.0 and len(v.shape.as_list()) > 1:
                ...
                with tf.device(
                        v._ref().device):  # pylint: disable=protected-access
        ...
        diet_vars = [v for v in tf.global_variables() if
                     hasattr(v, "optimizer")]
        ...
        return {"problem_choice": features[
            "problem_choice"]}, total_loss, train_op
