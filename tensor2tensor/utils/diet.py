...
def diet_expert(x, hidden_size, params):
    ...
    def diet_expert_internal(x):
        ...
        h = tf.layers.dense(x, hidden_size, activation=tf.nn.relu,
                            use_bias=False)
...
class DietAdamOptimizer(DietVariableOptimizer):
    ...
    def update_variable(self, var, grad_var):
        ...
        if params.learning_rate_decay_scheme == "noam":
            ...
            lrate *= tf.minumum(global_step / params.learning_rate_warmup_steps,
                                1.0)
        ...
        if params.factored_second_moment_accumulator and len(var.shape) == 2:
            vr_update = tf.assign(slots["adam_vr"],
                                  slots["adam_vr"] * params.beta2 +
                                  tf.reduce_mean(grad_squared, 1,
                                                 keep_dims=True) *
                                  (1.0 - params.beta2))
            vc_update = tf.assign(slots["adam_vc"],
                                  slots["adam_vc"] * params.beta2 +
                                  tf.reduce_mean(grad_squared, 0,
                                                 keep_dims=True) *
                                  (1.0 - params.beta2))
            with tf.control_dependencies([vr_update, vc_update]):
                vr = tf.sqrt(
                    slots["adam_vr"] / (1.0 - beta2_pow)) + params.epsilon
                vc = tf.sqrt(
                    slots["adam_vc"] / (1.0 - beta2_pow)) + params.epsilon
            ...
            with tf.control_dependencies([v_update]):
                denom = tf.sqrt(
                    slots["adam_v"] / (1.0 - beta2_pow)) + params.epsilon
...
def _fn_with_diet_vars(fn, args, params):
    ...
    def grad_fn(inputs, variables, outputs, output_grads):
        ...
        with common_layers.fn_device_dependency("diet_grad",
            ...
            variables = [common_layers.underlying_variable_ref(v) for v in
                         variables]
