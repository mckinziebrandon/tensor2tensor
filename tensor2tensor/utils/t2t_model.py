...
class T2TModel(object):
    ...
    def _create_modalities(self, problem_hparams, hparams):
        ...
        if target_modality_name:
            _warn_changed_modality_type(target_modality_name,
                                        target_modality_spec[0],
                                        "target")
            target_modality_spec = (
            target_modality_name, target_modality_spec[1])
    ...
    def _beam_decode(self, features, decode_length, beam_size, top_beams,
        ...
        def symbols_to_logits_fn(ids):
            ...
            current_output_position = tf.shape(ids)[
                                          1] - 1  # -1 due to the pad above.
        ...
        decode_length = tf.shape(features["inputs"])[1] + tf.constant(
            decode_length)
    ...
    def sample(self, features, last_position_only=False):
        ...
        if self._hparams.sampling_method == "argmax":
            sharded_samples = self._data_parallelism(tf.argmax, sharded_logits,
                                                     4)
        else:
    ...
    def model_fn(self, features, skip=False, last_position_only=False):
        ...
        if "target_space_id" in features:
            transformed_features["target_space_id"] = [features[
                                                           "target_space_id"]
                                                       ] * self._num_datashards
        ...
        with tf.variable_scope(target_modality.name, reuse=target_reuse):
            if not last_position_only:
                sharded_logits = target_modality.top_sharded(
                    body_outputs, sharded_features["targets"],
                    self._data_parallelism)
                training_loss = target_modality.loss_sharded(
                    sharded_logits, sharded_features["targets"],
                    self._data_parallelism)
        ...
        tf.logging.info(
            "This model_fn took %.3f sec." % (time.time() - start_time))
            ...
            if isinstance(output, tuple):
                ...
                if isinstance(losses_sharded[0], dict):
                    ...
                    for k in losses_sharded[0].keys():
                        k_loss_sharded = [losses[k] for losses in
                                          losses_sharded]
