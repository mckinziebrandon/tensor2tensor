...
def decode_from_dataset(estimator,
    ...
    for problem_idx, problem_name in enumerate(problem_names):
        ...
        tf.logging.info(
            "Completed inference on %d samples." % num_predictions)  # pylint: disable=undefined-loop-variable
...
def decode_from_file(estimator, filename):
    ...
    for _ in range(num_decode_batches):
        ...
        for result in result_iter:
            ...
            def log_fn(inputs, outputs):
                ...
                tf.logging.info(
                    "Inference results OUTPUT: %s" % decoded_outputs)
    ...
    decode_filename = (
    base_filename + "." + FLAGS.model + "." + FLAGS.hparams_set
...
def decode_interactively(estimator):
    ...
    for problem_idx, example in infer_input_fn:
        ...
        for result in result_iter:
            if FLAGS.decode_return_beams:
                beams = np.split(result["outputs"], FLAGS.decode_beam_size,
                                 axis=0)
                scores = None
                if "scores" in result:
                    scores = np.split(result["scores"], FLAGS.decode_beam_size,
                                      axis=0)
                for k, beam in enumerate(beams):
                    tf.logging.info("BEAM %d:" % k)
                    beam_string = targets_vocab.decode(
                        _save_until_eos(beam.flatten()))
                    if scores is not None:
                        tf.logging.info(
                            "%s\tScore:%f" % (beam_string, scores[k]))
                    else:
                        tf.logging.info(beam_string)
            else:
                if FLAGS.identity_output:
                    tf.logging.info(
                        " ".join(map(str, result["outputs"].flatten())))
                else:
                    tf.logging.info(
    ...
    while True:
        ...
        if input_string == "q":
            ...
            vocabulary = p_hparams.vocabulary[
                "inputs" if has_input else "targets"]
            ...
            problem = int(input_string[3:])
            p_hparams.target_modality = hparams.problems[
                problem].target_modality
            p_hparams.target_space_id = hparams.problems[
                problem].target_space_id
...
def _get_sorted_inputs(filename):
    ...
    input_lens = [(i, len(line.strip().split())) for i, line in
                  enumerate(inputs)]
