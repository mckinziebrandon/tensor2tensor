...
def standardize_images(x):
    ...
    with tf.name_scope("standardize_images", [x]):
        ...
        if x.shape and len(x.shape) == 4 and x.shape[3] == 1:
            x = tf.concat([x, x, x],
                          axis=3)  # Not used, just a dead tf.cond branch.
...
def embedding(x, vocab_size, dense_size, name=None, reuse=None, multiplier=1.0):
    ...
    with tf.variable_scope(
        ...
        return tf.reshape(emb_x,
                          [shape[0], shape[1], shape[2], static_shape[4]])
...
def conv_stride2_multistep(x, nbr_steps, output_filters, name=None, reuse=None):
    ...
    """
    with tf.variable_scope(
            name, default_name="conv_stride2_multistep", values=[x],
            reuse=reuse):
        if nbr_steps == 0:
...
def deconv_stride2_multistep(x,
    ...
    """
    with tf.variable_scope(
            name, default_name="deconv_stride2_multistep", values=[x],
            reuse=reuse):
...
def separable_conv(inputs, filters, kernel_size, **kwargs):
    return conv_internal(tf.layers.separable_conv2d, inputs, filters,
                         kernel_size,
                         **kwargs)
    ...
    def conv_fn(inputs, filters, kernel_size, **kwargs):
        ...
        if separability:
            ...
            for split_idx, split in enumerate(
                    tf.split(inputs, abs_sep, axis=3)):
                with tf.variable_scope("part_%d" % split_idx):
                    if separability > 0:
                        parts.append(
                            tf.layers.conv2d(split, filters // separability,
                                             kernel_size, **
                                             kwargs))
                    else:
                        parts.append(
                            tf.layers.separable_conv2d(split,
                                                       filters // abs_sep,
                                                       kernel_size, **kwargs))
            if separability > 1:
                result = tf.layers.conv2d(tf.concat(parts, axis=3), filters,
                                          (1, 1))
    ...
    with tf.name_scope("pool", [inputs]):
        ...
        if padding == "LEFT":
            ...
            if len(static_shape) == 3:
                ...
                width_padding = 0 if static_shape[2] == 1 else cond_padding
                padding_ = [[0, 0], [height_padding, 0], [width_padding, 0],
                            [0, 0]]
            inputs = tf.pad(inputs, padding_)
            inputs.set_shape([static_shape[0], None, None, static_shape[3]])
            padding = "VALID"

    return tf.nn.pool(inputs, window_size, pooling_type, padding,
                      strides=strides)
...
def conv_block_downsample(x,
    ...
    with tf.variable_scope(
            name, default_name="conv_block_downsample", values=[x],
            reuse=reuse):
...
def decompress_seqcnn(x,
    ...
    with tf.variable_scope(
        ...
        targets_shape, targets_shape_static = tf.shape(
            targets), targets.get_shape()
...
def simple_attention(target, source, bias=None):
    ...
    with tf.name_scope("simple_attention", [target, source]):
        ...
        if not tf.get_variable_scope().reuse:
            tf.summary.image("attention", tf.expand_dims(attention, 3),
                             max_outputs=5)
...
def multiscale_conv_and_attention(x, padding, hparams, source=None):
    ...
    conv_sum = multiscale_conv_sum(
        x,
        hparams.hidden_size,
        [((hparams.kernel_height ** i, hparams.kernel_width **
...
def get_timing_signal(length,
    ...
    scaled_time = tf.expand_dims(positions, 1) * tf.expand_dims(inv_timescales,
                                                                0)
...
def attention_1d_v0(source,
    ...
    with tf.variable_scope(name, default_name="attention", values=[target]):
        ...
        attention = tf.matmul(target_attention, source_attention,
                              transpose_b=True)
        ...
        if not tf.get_variable_scope().reuse:
            ...
            image = tf.reshape(image, [
                batch, target_length, source_length, 3,
                (num_heads + extra_heads) // 3
            ])
...
def conv_hidden_relu(inputs,
    ...
    with tf.variable_scope(name, "conv_hidden_relu", [inputs]):
        ...
        ret = conv_f2(h, output_size, second_kernel_size, name="conv2",
                      **kwargs)
        if is_3d:
...
def linear_set_layer(layer_size,
    ...
    with tf.variable_scope(
        ...
        outputs = conv1d(inputs, layer_size, 1, activation=None,
                         name="set_conv")
...
def smoothing_cross_entropy_factored_grad(op, dy):
    ...
    for part in xrange(num_splits):
        with tf.control_dependencies(deps):
            ...
            output_part = smoothing_cross_entropy(logits, labels[part],
                                                  vocab_size,
                                                  confidence)
...
def smoothing_cross_entropy_factored(a, b, labels, confidence):
    ...
    for part in xrange(num_splits):
        with tf.control_dependencies(parts[-1:]):
            logits = tf.matmul(a[part], b, transpose_b=True)
            parts.append(
                smoothing_cross_entropy(logits, labels[part], vocab_size,
                                        confidence))
...
def _fn_with_custom_grad(fn, inputs, grad_fn, use_global_vars=False):
    ...
    if grad_fn is None:
        ...
        def custom_grad_fn(op, *dys):
            ...
            grad_inputs, grad_vars = grad_fn(fn_inputs, fn_vars, fn_outputs,
                                             dys)
    ...
    if not forget:
        ...
        def grad_fn(x, f1, f2, scale, bias, dy):
            with tf.control_dependencies([dy]):
                ...
                for i in xrange(num_splits):
                    with tf.control_dependencies(deps):
                        n = layer_norm_compute_python(xs[i], epsilon, scale,
                                                      bias)
                        y = tf.nn.conv1d(n, f1, 1, "SAME")
                        y = tf.nn.relu(y)
                        y = tf.nn.conv1d(y, f2, 1, "SAME")
                        dxi, pdf1, pdf2, pdscale, pdbias = tf.gradients(
                            ys=[y], xs=[xs[i], f1, f2, scale, bias],
                            grad_ys=[dys[i]])
                        df1 += pdf1
                        df2 += pdf2
                        dscale += pdscale
