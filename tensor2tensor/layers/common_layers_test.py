...
class CommonLayersTest(tf.test.TestCase):
            ...
            with self.test_session() as session:
                with tf.variable_scope("sep_%d" % sep):
                    y = common_layers.subseparable_conv(
                        tf.constant(x, dtype=tf.float32), 16, (3, 1),
                        separability=sep)
                session.run(tf.global_variables_initializer())
                res = session.run(y)
    ...
    def testGetTimingSignal(self):
        ...
        num_timescales = 10
        with self.test_session() as session:
            a = common_layers.get_timing_signal(length,
    ...
    def testAddTimingSignal(self):
        ...
        with self.test_session() as session:
            a = common_layers.add_timing_signal(
                tf.constant(x, dtype=tf.float32))
            session.run(tf.global_variables_initializer())
            res = session.run(a)
    ...
    def testConvGRU(self):
        ...
        with self.test_session() as session:
            y = common_layers.conv_gru(tf.constant(x, dtype=tf.float32), (1, 3),
                                       11)
    ...
    def testConvLSTM(self):
        ...
        with self.test_session() as session:
            y = common_layers.conv_lstm(tf.constant(x, dtype=tf.float32),
                                        (1, 3), 13)
            session.run(tf.global_variables_initializer())
            res = session.run(y)
    ...
    def testApplyNormLayer(self):
        with self.test_session() as session:
            ...
            x2 = common_layers.apply_norm(
                tf.constant(x1, dtype=tf.float32), "layer", depth=11,
                epsilon=1e-6)
            session.run(tf.global_variables_initializer())
    ...
    def testApplyNormNoam(self):
        with self.test_session() as session:
            x1 = np.random.rand(5, 2, 1, 11)
            x2 = common_layers.apply_norm(
                tf.constant(x1, dtype=tf.float32), "noam", depth=11,
                epsilon=1e-6)
            session.run(tf.global_variables_initializer())
            actual = session.run(x2)
        self.assertEqual(actual.shape, (5, 2, 1, 11))
    ...
    def testApplyNormBatch(self):
        with self.test_session() as session:
            x1 = np.random.rand(5, 2, 1, 11)
            x2 = common_layers.apply_norm(
                tf.constant(x1, dtype=tf.float32), "batch", depth=11,
                epsilon=1e-6)
            session.run(tf.global_variables_initializer())
            actual = session.run(x2)
        self.assertEqual(actual.shape, (5, 2, 1, 11))
    ...
    def testApplyNormNone(self):
        with self.test_session() as session:
            x1 = np.random.rand(5, 2, 1, 11)
            x2 = common_layers.apply_norm(
                tf.constant(x1, dtype=tf.float32), "none", depth=11,
                epsilon=1e-6)
            session.run(tf.global_variables_initializer())
            actual = session.run(x2)
        self.assertEqual(actual.shape, (5, 2, 1, 11))
    ...
    def testPaddingCrossEntropyFactored(self):
        ...
        with self.test_session() as session:
            ...
            logits = tf.matmul(
                tf.reshape(features, [rows * cols, depth]), weights,
                transpose_b=True)
    ...
    def testFactoredTensorImplicitConversion(self):
        ...
        with self.test_session() as session:
            ...
            factored = common_layers.FactoredTensor(tf.to_float(a),
                                                    tf.to_float(b))
    ...
    def testConvHiddenReluMemoryEfficient(self):
        ...
        with self.test_session() as session:
            ...
            dx_f, df1_f, df2_f, dnorm_scale_f, dnorm_bias_f = tf.gradients(
                ys=[y_forget], xs=[x, f1, f2, norm_scale, norm_bias],
                grad_ys=[dy])
            session.run(tf.global_variables_initializer())
            (y, y_forget,
...
class FnWithCustomGradTest(tf.test.TestCase):
    ...
    def testCustomGrad(self):
        ...
        def grad_fn(inputs, variables, unused_outputs, unused_grad_outputs):
            grad_inputs = [tf.ones_like(t) * (i + 1.) for i, t in
                           enumerate(inputs)]
            grad_vars = [
