    ...
    def top(self, body_output, _):
        with tf.variable_scope("rgb_softmax"):
            ...
            output_rgb_embedding_var = tf.get_variable(
                ...
                [self._channels, self.top_dimensionality,
                 self._body_input_depth],
            ...
            for i in self._channels:
                ...
                rgb_channel_logits.append(
                    tf.reshape(channel_logits,
                               tf.concat([shape, [self.top_dimensionality]],
                                         0)))
    ...
    def bottom(self, inputs):
        ...
        with tf.variable_scope(self.name):
            def xnet_resblock(x, filters, res_relu, name):
                with tf.variable_scope(name):
                    y = common_layers.separable_conv_block(
                        ...
                        force2d=True,
                        name="sep_conv_block")
                    y = common_layers.pool(y, (3, 3), "MAX", "SAME",
                                           strides=(2, 2))
            ...
            x = common_layers.conv_block(
                ...
                name="conv0")
            x = common_layers.conv_block(
                x, 64, [((1, 1), (3, 3))], padding="SAME", force2d=True,
                name="conv1")
    ...
    def top(self, body_output, _):
        with tf.variable_scope("real"):
            return tf.nn.softplus(
                tf.layers.dense(body_output, self._vocab_size))
