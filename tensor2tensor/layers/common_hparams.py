        ...
        for ctr, ctr_name in ctr_names:
            ...
            if name in ctr:
                raise ValueError("Setting hyperparameter %s as type %s, but a "
                                 ...
                                 "registered as type %s" % (
                                 name, ctr_name, orig_name))
...
def fill_ranged_hparams_from_hparams(hparams, ranged_hparams):
    ...
    Raises:
      ValueError: if hparams contains a hyperparameter not of type
        {int, float, str, bool}.
    """
    for name, (hp_type, is_multivalent) in six.iteritems(
            hparams._hparam_types):  # pylint: disable=protected-access

        if is_multivalent:
            raise ValueError(
                "Multivalent hparams not supported in RangedHParams. "
        ...
        if hp_type == int:
            ...
            raise ValueError(
                "Unsupported type %s for param %s" % (hp_type, name))
...
def basic_range1(ranged_hparams):
    ...
    rhp.set_discrete("hidden_size", [32, 64, 128, 256, 512],
                     scale=rhp.LOG_SCALE)
