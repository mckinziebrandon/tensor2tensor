...
def shake_shake_block_branch(x, conv_filters, stride):
    ...
    x = tf.nn.relu(x)
    x = tf.layers.conv2d(x, conv_filters, (3, 3), strides=(1, 1),
                         padding="SAME")
    x = tf.layers.batch_normalization(x)
    return x
...
class ShakeShake(t2t_model.T2TModel):
    ...
    def model_fn_body(self, features):
        ...
        with tf.variable_scope("shake_shake_stage_2"):
            x = shake_shake_stage(x, blocks_per_stage, hparams.base_filters * 2,
                                  2,
                                  hparams)
        with tf.variable_scope("shake_shake_stage_3"):
            x = shake_shake_stage(x, blocks_per_stage, hparams.base_filters * 4,
                                  2,
                                  hparams)
