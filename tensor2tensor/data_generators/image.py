...
class ImageCeleba(ImageProblem):
    ...
    def generator(self, tmp_dir, how_many, start_from=0):
        ...
        for fname, url in [self.IMG_DATA, self.LANDMARKS_DATA, self.ATTR_DATA]:
            path = generator_utils.maybe_download_from_drive(tmp_dir, fname,
                                                             url)
        ...
        for url in fsns_files:
            if "/train/train" in url:
                generator_utils.maybe_download(
                    data_dir,
                    "image_fsns-train" + url[-len("-00100-of-00512"):], url)
            elif "/validation/validation" in url:
                generator_utils.maybe_download(
                    data_dir, "image_fsns-dev" + url[-len("-00100-of-00512"):],
                    url)
            elif "charset" in url:
                generator_utils.maybe_download(data_dir, "charset_size134.txt",
                                               url)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            for (image, label) in zip(images, labels):
                enc_string = sess.run(encoded_image_t,
...
def _get_mnist(directory):
    ...
    for filename in [
        ...
        generator_utils.maybe_download(directory, filename,
                                       _MNIST_URL + filename)
...
def _extract_mnist_images(filename, num_images):
    ...
    with gzip.open(filename) as bytestream:
        bytestream.read(16)
        buf = bytestream.read(
            _MNIST_IMAGE_SIZE * _MNIST_IMAGE_SIZE * num_images)
...
def cifar10_generator(tmp_dir, training, how_many, start_from=0):
    ...
    for filename in data_files:
        ...
        images = images.reshape((num_images, 3, _CIFAR10_IMAGE_SIZE,
                                 _CIFAR10_IMAGE_SIZE))
        all_images.extend([
            np.squeeze(images[j]).transpose((1, 2, 0)) for j in
            xrange(num_images)
        ])
        labels = data["labels"]
